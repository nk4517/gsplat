...
from examples.lib_dog import fast_dog
...
def compute_skysphere_geometry_dog(trainset, skysphere_radius, num_points, device, sigma=2.0, k=3.6):
    """Compute skysphere geometry and colors from camera views using DoG-weighted sampling.
    
    Returns points, colors, and quaternions for skysphere initialization.
    """

    all_colors, all_points = reproject_skysphere_dog(trainset, skysphere_radius, num_points, device, sigma, k)
...
def reproject_skysphere_dog(trainset, skysphere_radius, samples, device, sigma=2.0, k=3.6):
    """Reproject skysphere with DoG-weighted sampling.
    
    Creates fibonacci sphere with 20x points, uses DoG as weights for random sampling of 1/20 visible points.
    """

    # Generate fibonacci sphere with 20x points
    skysphere_pts3d = fibonacci_sphere(
        samples=samples * 20,
        radius=skysphere_radius
    ).float().to(device)
    ...
    for i in tqdm.trange(len(trainset), desc="Initializing skysphere (DoG)"):
        data = trainset[i]
        camtoworld = data["camtoworld"].to(device).unsqueeze(0)
        K = data["K"].to(device).unsqueeze(0)
        image = data["image"].to(device) / 255.0
        ...
        dog_map = fast_dog(image_chw, sigma=sigma, k=k).squeeze(0).squeeze(0)
        ...
        # Sample colors and check sky mask
        visible_unpopulated_xy = points_2d[visible_unpopulated].long()
        un_y = visible_unpopulated_xy[:, 1].clamp(0, height - 1)
        un_x = visible_unpopulated_xy[:, 0].clamp(0, width - 1)
        ...
        # Get DoG weights for visible unpopulated sky points
        dog_weights = dog_map[un_y, un_x]
        
        # Sample 1/20 of visible points using DoG as weights
        num_visible = visible_unpopulated.sum().item()
        num_to_sample = max(1, num_visible // 20)
        
        if dog_weights.sum() > 0:
            sampled_indices = torch.multinomial(
                dog_weights,
                num_samples=min(num_to_sample, num_visible),
                replacement=False
            )
        else:
            sampled_indices = torch.randperm(num_visible, device=device)[:num_to_sample]

        # Get global indices of sampled points
        visible_unpopulated_indices = torch.where(visible_unpopulated)[0]
        sampled_global_indices = visible_unpopulated_indices[sampled_indices]

        # Extract sampled points and colors
        pts3d_to_add = skysphere_pts3d[sampled_global_indices]
        colors_to_add = image[un_y[sampled_indices], un_x[sampled_indices]]

        all_points.append(pts3d_to_add)
        all_colors.append(colors_to_add)
        populated[sampled_global_indices] = True
